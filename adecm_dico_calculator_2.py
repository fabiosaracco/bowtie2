...
def main():
    ...
    for i in range(len(files)//2):
        ...
        for dico_class in dicos:
            ...
            #    continue
            adecm_filename=TEST_FOLDER+f'{dataset_name}_dico{dico_class}_adecm.pkl'
            if os.path.exists(adecm_filename):
                with open(adecm_filename, 'rb') as f:
                    model=pickle.load(f)
                ...
                if hasattr(model, 'sol_weights') and hasattr(model.sol_weights, 'converged'):
                    ...
                    try:
                        if adecm.N<10**4:
                            # in this case, the use of the gauge was not effective, in the previous tests and may interfere with convergence, so we try to solve without it
                            adecm.solve_tool(tol=1e-5, backend='pytorch', max_time=MAX_TIME_HOURS*3600, verbose=True, monitor=True)
