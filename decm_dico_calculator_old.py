    ...
    for i in range(len(files)//2):
        ...
        for dico_class in dicos:
            ...
            if decm.converged:
                ...
                if not os.path.exists(qdecm_filename):
                    print(f'[{dt.datetime.now():%Y-%m-%d %H:%M:%S}] Trying qDECM...')
                    print(f'[{dt.datetime.now():%Y-%m-%d %H:%M:%S}] qDECM, pytorch, theta (max: {MAX_TIME_HOURS:} hours)')
                    qdecm=qDECMModel(aux[0], aux[1], aux[2], aux[3])

                    try:
                        qdecm.solve_tool(tol=1e-6, backend='pytorch', max_time=MAX_TIME_HOURS*3600)
                    except Exception as e:
                        print(f'[{dt.datetime.now():%Y-%m-%d %H:%M:%S}] Error solving QDECM with pytorch and theta: {e}')
                    # with backend='pytorch'
                    with open(HOME+f'/test/{dataset_name}_dico{dico_class}_qdecm.pkl', 'wb') as f:
                        pickle.dump(qdecm, f)
                    # elapsed time (in hours and minutes)
                    ...
                    if qdecm.sol_topo.converged and qdecm.sol_weights.converged:
                        print(f'[{dt.datetime.now():%Y-%m-%d %H:%M:%S}] QDECM converged in {eth:2d} h and {etm:2d} m, MRE(degrees)={qdecm.constraint_error_topology(qdecm.sol_topo.theta):.2e}, MRE(strengths)={qdecm.constraint_error_strength(qdecm.sol_topo.theta, qdecm.sol_weights.theta):.2e} (peak RAM={qdecm.sol_topo.peak_ram_bytes//1024**2} MB (topo), {qdecm.sol_weights.peak_ram_bytes//1024**2} MB (weights))')
                    else:
                        print(f'[{dt.datetime.now():%Y-%m-%d %H:%M:%S}] QDECM did not converge in {eth:2d} h and {etm:2d} m, MRE(degrees)={qdecm.constraint_error_topology(qdecm.sol_topo.theta):.2e}, MRE(strengths)={qdecm.constraint_error_strength(qdecm.sol_topo.theta, qdecm.sol_weights.theta):.2e} (peak RAM={qdecm.sol_topo.peak_ram_bytes//1024**2} MB (topo), {qdecm.sol_weights.peak_ram_bytes//1024**2} MB (weights))')
